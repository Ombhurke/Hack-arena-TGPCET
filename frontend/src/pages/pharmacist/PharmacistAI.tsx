                            ...
                            {!m.isUser && m.steps && m.steps.length > 0 && (
                                <div className="mt-3 pt-3 border-t border-slate-100">
                                    <details className="group">
                                        <summary className="text-[11px] font-semibold text-slate-400 cursor-pointer hover:text-[#B02BE0] transition-colors list-none flex items-center gap-1">
                                            <ChevronDown className="w-3 h-3 group-open:rotate-180 transition-transform" />
                                            <span>Show Reasoning & Steps ({m.steps.length})</span>
                                        </summary>
                                        <div className="mt-2 space-y-2 bg-slate-50 rounded-lg p-2.5 border border-slate-100">
                                            {m.steps.map((s, idx) => (
                                                <div key={idx} className="flex gap-2 items-start">
                                                    <div className={`mt-1 w-1.5 h-1.5 rounded-full shrink-0 ${s.agent === 'thought' ? 'bg-amber-400' : 'bg-[#B02BE0]'}`} />
                                                    <div className="flex flex-col">
                                                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                                                            {s.agent === 'thought' ? '💭 Reasoning' : `🤖 ${s.agent}`}
                                                        </span>
                                                        <p className="text-[12px] text-slate-600 leading-snug">{s.message}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
